...
class Wdseo_Robots_Meta {
    ...
    public static function get_blocked_patterns() {
        if (self::$blocked_patterns === null) {
            $patterns = get_option('wdseo_robots_blocked_urls', '');
    ...
    public static function output_robots_meta_tag() {
        ...
        if (is_singular()) {
            ...
            $robots = $directive !== 'default' ? $directive : 
                     Wdseo_Settings::get_setting("wdseo_default_robots_" . $post->post_type, 'index,follow');
        } elseif (is_tax() || is_category() || is_tag()) {
            $term = get_queried_object();
            $directive = get_term_meta($term->term_id, '_wdseo_term_robots_directive', true);
            $robots = $directive !== '' ? $directive : 
                     Wdseo_Settings::get_setting("wdseo_default_robots_" . $term->taxonomy, 'index,follow');
        } elseif (is_author()) {
            $robots = Wdseo_Settings::get_setting('wdseo_default_robots_author', 'noindex,follow');
        } elseif (is_front_page() || is_home()) {
            $robots = Wdseo_Settings::get_setting('wdseo_default_robots_home', 'index,follow');
        }
