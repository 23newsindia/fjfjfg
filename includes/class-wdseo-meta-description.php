...
class Wdseo_Meta_Description {
    ...
    $og_desc = get_post_meta($post->ID, '_wdseo_og_description', true);
...
public static function edit_term_description_field($term, $taxonomy) {
    ...
    $og_desc = get_term_meta($term->term_id, '_wdseo_term_og_description', true);
...
public static function render_home_og_description_field() {
    $desc = get_option('wdseo_home_og_description', '');
     ...
     */
    // In class-wdseo-meta-description.php
    
    
public static function output_meta_description() {
    if (!get_option('wdseo_enable_meta_description', 1)) {
        return;
    }
    ...
    if (is_singular()) {
        ...
        $desc = get_term_meta($term_id, '_wdseo_term_description', true);
        $og_desc = get_term_meta($term_id, '_wdseo_term_og_description', true);
    } elseif (is_front_page() || is_home()) {
        $desc = get_option('wdseo_home_meta_description', '');
        $og_desc = get_option('wdseo_home_og_description', '');
    }
