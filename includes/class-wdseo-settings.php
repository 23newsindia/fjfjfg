...
class Wdseo_Settings {
    ...
    public static function init() {
        ...
        add_filter('pre_get_document_title', array(__CLASS__, 'maybe_remove_site_name_from_title'), 999);
    }
...
if (!is_array($checked)) {
    ...
    public static function sanitize_textarea_input($input) {
      ...
      /**
     * Remove site name from title if enabled for current page type
     */
     
     
    public static function maybe_remove_site_name_from_title($title) {
    // Don't modify title in admin or REST requests
    if (is_admin() || defined('REST_REQUEST')) {
        return $title;
    }

    // Map query conditionals to our setting keys
    $type_map = array(
        'is_singular' => 'post', // Handles all singular content
        'is_page' => 'page',
        'is_singular_product' => 'product',
        'is_tax_product_cat' => 'product_cat',
        'is_front_page' => 'home'
    );
    ...
    $current_type = false;
    foreach ($type_map as $conditional => $key) {
        if ($current_type === false && function_exists($conditional) && call_user_func($conditional)) {
            $current_type = $key;
            break;
        }
    }

    // If current type should remove site name
    if ($current_type && in_array($current_type, $remove_from)) {
        $site_name = get_bloginfo('name');
        $clean_title = wp_strip_all_tags($title);
        
        // Find position of site name in title
        $pos = strrpos($clean_title, $site_name);
        
        if ($pos !== false) {
            // Calculate separator position (before site name)
            $separator_pos = strrpos($clean_title, ' ', - (strlen($clean_title) - $pos + 1));
            
            if ($separator_pos !== false) {
                // Remove separator and site name
                return substr($clean_title, 0, $separator_pos);
            }
            
            // If no separator found, just remove site name
            return str_replace($site_name, '', $clean_title);
        }
    }
